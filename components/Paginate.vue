...
const paginator = reactive({
  array: props.players.slice(sliceIdx.value[0], sliceIdx.value[1]),
})
watch(
  () => currentPage.value,
  () => {
...
function back() {
  ...
  else currentPage.value = Math.floor(props.players.length / perPage)
}
function next() {
  if (currentPage.value < Math.floor(props.players.length / perPage))
    currentPage.value++
